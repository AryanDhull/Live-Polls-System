...
function Timer({ seconds }) {
  ...
  return (
    <div className="text-black-100">
      <span className="border border-1 border-black-100 p-3 rounded-full">
        {countdown} / {seconds}
      </span>{" "}
      seconds left
    </div>
    ...
    <div>
      ...
      {showWaitingMessage && (
        <div className="flex flex-col items-center justify-center h-screen gap-4">
          <h1 className="text-2xl text-black-100 block">
        ...
        <div>
          <div>
            ...
            <div className="p-8 flex flex-col items-center ml-[40px]">
              <h2 className="mt-3 mb-3 text-black-50 text-lg">
                Q. {question.question} ?
              </h2>
              <div className="ml-2">
                {question.options.map((opt, index) => (
                  <div key={index} className="mb-2 flex gap-3">
                    ...
                    />
                    <label className="text-black text-md">{opt.text}</label>
                  </div>
                ))}
                <button
                  type="submit"
                  onClick={handleSubmitAnswer}
                  className="rounded border-2 p-3 mt-2 border-black-700 bg-slate-50 hover:bg-slate-300"
                >
                  Submit
                </button>
              </div>
