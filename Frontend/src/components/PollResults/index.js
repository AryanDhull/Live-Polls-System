  ...
  return (
    <div>
      <div className="flex justify-between items-center p-4">
        <h1 className="text-black text-bold text-lg">Polling results</h1>
      </div>
    
      <hr />
      {question ? (
        <div className="p-4 flex flex-col items-center mt-[40px]">
          <h1 className=" text-black-100 text-2xl">{question} ?</h1>
          <ul className="mt-4 w-1/2 mb-4">
            {optionResults.map((option, index) => (
              <li
                id={`option-${index}`}
                className={
                  "flex justify-between mb-4 border-2 bg-lime-100 border-slate-700 p-3 rounded hover:bg-lime-200"
                }
                key={index}
              >
          ...
          {teacherMode ? (
            <div className="flex justify-between w-1/2 mt-2">
              ...
              <button className="border-2 bg-yellow-100 border-slate-700 p-2 rounded hover:bg-yellow-200 mr-2">
             <Link to='/pastPolls'>pastPools</Link> 
            </button>
            </div>
          ) : (
